...
:build
	...
	call set CFLAGS=%%CFLAGS%% %%CFLAGS_%PLATFORM%%%

	echo Compiling %TARGET%...
	echo %CC% %CFLAGS% %INCLUDE% %LIBS% %SOURCE_FILES% -Wl,-Map=%TARGET%.map -o %TARGET%.elf
	...
	echo %OBJCOPY% -R .noinit -O ihex %TARGET%.elf %TARGET%.hex
	     %OBJCOPY% -R .noinit -O ihex %TARGET%.elf %TARGET%.hex                              || goto error
	echo %OBJCOPY% -R .noinit -O binary %TARGET%.elf %TARGET%.bin
	     %OBJCOPY% -R .noinit -O binary %TARGET%.elf %TARGET%.bin                            || goto error
